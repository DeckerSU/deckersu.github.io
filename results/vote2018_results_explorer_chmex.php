...
// read chmex snapshot data
if ($file = fopen("6700.VOTE2018.height.csv", "r")) {
    $line_index = 0;
    while(!feof($file)) {
    $line = fgets($file);
    if (($line)) {
        $candidate = explode(",", trim($line));    
        $chmex_snapshot_data[trim($candidate[0])] = trim($candidate[1]);
    }
    $line_index++;
    }
}
...
foreach ($json_data as $address => $candidate) {
    ...
    if (array_key_exists($address,$chmex_snapshot_data)) {
	$balance_chmex_snapshot = $chmex_snapshot_data[$address];
    }
    echo '
         <tr>
	    <td>'.$region.'</td>
